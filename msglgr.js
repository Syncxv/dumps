async function msglgr() {
  
  let fuck =  class MessageLoggerV2 {
    getName() {
    ...
    start() {
      let onLoaded = () => {
        try {
          if (!window.ZeresPluginLibrary || !(this.localUser = ZeresPluginLibrary.DiscordModules.UserStore.getCurrentUser())) setTimeout(onLoaded, 1000);
      ...
      this.__isPowerCord = !!window.powercord && typeof BdApi.__getPluginConfigPath === 'function' || typeof window.isTab !== 'undefined';
      let XenoLibOutdated = false;
      let ZeresPluginLibraryOutdated = false;
      if (window.BdApi && BdApi.Plugins && typeof BdApi.Plugins.get === 'function' /* you never know with those retarded client mods */) {
        ...
        const isOutOfDate = (lib, minVersion) => lib && lib._config && lib._config.info && lib._config.info.version && versionChecker(lib._config.info.version, minVersion) || typeof window.isTab !== 'undefined';
      ...
      if (!window.XenoLib || !window.ZeresPluginLibrary || window.DiscordJS || XenoLibOutdated || ZeresPluginLibraryOutdated) {
        this._XL_PLUGIN = true;
        if ("undefined" != typeof window.isTab) return;
        ...
        const c = !window.XenoLib,
          d = !window.ZeresPluginLibrary,
          ...
          try {
            return BdApi.React.createElement(k, {
              ...
              onConfirm: () => {
                ...
                //   g = () => window.XenoLib && !XenoLibOutdated ? (BdApi.isSettingEnabled("fork-ps-5") || BdApi.isSettingEnabled("autoReload")) && !a ? void 0 : void BdApi.showToast("Reload to load the libraries and plugin!") : void c("https://raw.githubusercontent.com/1Lighty/BetterDiscordPlugins/master/Plugins/1XenoLib.plugin.js", (c, g, h) => {
                //     try {
                ...
                //     }
                //   });
                // !window.ZeresPluginLibrary || ZeresPluginLibraryOutdated ? c("https://raw.githubusercontent.com/rauenzi/BDPluginLibrary/master/release/0PluginLibrary.plugin.js", (a, c, h) => {
                //   try {
  ...
  */
  ...
  //     if (!this.settings.cacheAllImages ) {
  //       try {
  ...
  //   }
    ...
    efficientlySaveData(name, key, data, callback) {
      try {
        ...
        console.log('no saving in browser nigga like ongod')
      } catch (e) {
        XenoLib.Notifications.error('There has been an error saving the data file');
    ...
    cleanupMessageObject(message) {
      const ret = {
        ...
        content: window.ohgodohfuck ? '' : message.content,
        type: message.type,
    ...
    onDispatchEvent(args, callDefault) {
      ...
      try {
        ...
        if (dispatch.type == 'MESSAGE_DELETE') {
          ...
          this.saveData();
          return callDefault(...args);
        } else if (dispatch.type == 'MESSAGE_CREATE' && dispatch.message && (dispatch.message.content.length || (dispatch.attachments && dispatch.attachments.length) || (dispatch.embeds && dispatch.embeds.length)) && !window.ohgodohfuck && dispatch.message.state != 'SENDING' && !dispatch.optimistic && (dispatch.message.type === 0 || dispatch.message.type === 19)) {
          if (this.cachedMessageRecord.findIndex(m => m.id === dispatch.message.id) != -1) return callDefault(...args);
          this.cachedMessageRecord.push(dispatch.message);
